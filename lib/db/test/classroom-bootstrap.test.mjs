...
test("classroom target validation retains supported provider TLS options", () => {
  ...
  assert.equal(consumed.connectionParameters.host, target.hostname);
  assert.equal(consumed.connectionParameters.database, target.databaseName);
});

test("classroom target validation rejects malformed percent escapes in TLS options", () => {
  const databaseUrl =
    "postgresql://allowed.invalid/classroom%2Darchive?sslmode=verify%xxfull";
  const consumed = new Client({ connectionString: databaseUrl });
  assert.equal(consumed.connectionParameters.database, "classroom%2Darchive");
