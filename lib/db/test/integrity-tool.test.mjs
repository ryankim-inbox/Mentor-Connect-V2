    ...
    await assert.rejects(
      execFile(process.execPath, [cliPath, ...args], {
        ...
        env: {
          ...process.env,
          DATABASE_URL:
            "postgresql://migration-user:do-not-print@staging-db.invalid/mentor_connect",
      ...
      (error) =>
        ...
        !/do-not-print/.test(`${error.stdout}${error.stderr}`),
    );
  }
});
    ...
    execFile(
      ...
      [cliPath, "migration:preflight", "--env", "staging"],
      {
        ...
        env: {
          ...process.env,
          DATABASE_URL:
            "postgresql://reader:do-not-print@staging-db.invalid/mentor_connect",
          MIGRATION_ALLOWED_HOSTS_STAGING: "staging-db.invalid",
        },
      },
    ),
