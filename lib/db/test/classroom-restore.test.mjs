...
function fixtureConnectionUrl(base, username, password) {
...
test("validated native connection preserves TLS and cannot inherit another PG target", () => {
  ...
  const connection = sharedModule.classroomConnection(
    {
      databaseUrl: fixtureConnectionUrl(
        "postgresql://provider.invalid/classroom?sslmode=verify-full&sslrootcert=%2Ftmp%2Fprovider-ca.pem",
        "teacher",
        "private-canary",
      ),
...
integration(
  ...
  async (t) => {
    ...
    const encodedRoleBase = base.replace(
      "postgresql://",
      "postgresql://a%252Fb@",
    );
    ...
    try {
      await source.query('CREATE ROLE "a%2Fb" LOGIN SUPERUSER');
...
`,
      ...
      try {
        ...
        result = await backupModule.backupClassroom({
          databaseUrl: `${encodedRoleBase}/restore_source?ssl=false`,
      ...
      const restored = await restoreModule.restoreClassroom({
        databaseUrl: `${encodedRoleBase}/restore_empty?ssl=false`,
        allowedHost: "127.0.0.1",
...
test("native failures discard stderr containing URL and password canaries", async () => {
  ...
  try {
    ...
    await writeFile(
      executable,
      `#!/bin/sh
echo '${fixtureConnectionUrl("postgresql://host/db", "user", "password-canary")} secret-canary' >&2
exit 9
`,
      { mode: 0o700 },
    );
...
test("JavaScript and native clients disable TLS for false and absent options", () => {
  for (const query of ["?ssl=false", "?ssl=0", "?sslmode=disable", ""]) {
    const connection = sharedModule.classroomConnection({
      databaseUrl: fixtureConnectionUrl(
        `postgresql://provider.invalid:5433/classroom${query}`,
        "teacher",
        "password",
      ),
      allowedHost: "provider.invalid",
    ...
    const parameters = connection.createClient().connectionParameters;
    assert.equal(connection.env.PGSSLMODE, "disable");
    assert.equal(parameters.ssl, false);
    assert.equal(parameters.host, connection.env.PGHOST);
    assert.equal(parameters.port, Number(connection.env.PGPORT));
    assert.equal(parameters.database, connection.env.PGDATABASE);
  }
});

test("JavaScript and native clients decode percent-encoded identity exactly once", () => {
  const connection = sharedModule.classroomConnection({
    databaseUrl: fixtureConnectionUrl(
      "postgresql://provider.invalid/classroom?sslmode=disable",
      "a%2Fb",
      "p%2Fword",
    ),
    allowedHost: "provider.invalid",
  ...
  const parameters = connection.createClient().connectionParameters;
  assert.equal(connection.env.PGUSER, "a%2Fb");
  assert.equal(parameters.user, "a%2Fb");
  assert.equal(connection.env.PGPASSWORD, "p%2Fword");
  assert.equal(parameters.password, "p%2Fword");
});

test("unsupported TLS modes reject before either client can connect", () => {
  for (const mode of ["allow", "prefer", "require", "verify-ca"]) {
    assert.throws(
      () =>
        sharedModule.classroomConnection({
          databaseUrl: `postgresql://provider.invalid/classroom?sslmode=${mode}`,
          allowedHost: "provider.invalid",
          databaseName: "classroom",
        }),
      /TLS mode/,
...
test("JavaScript and native TLS settings preserve explicit provider CA verification", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "classroom-ca-"));
  try {
    const caPath = path.join(directory, "provider-ca.pem");
    await writeFile(caPath, "fixture provider certificate\n");
    const connection = sharedModule.classroomConnection({
      databaseUrl: `postgresql://provider.invalid/classroom?sslmode=verify-full&sslrootcert=${encodeURIComponent(caPath)}`,
      allowedHost: "provider.invalid",
      databaseName: "classroom",
    });
    const parameters = connection.createClient().connectionParameters;
    ...
    assert.equal(
      parameters.ssl.ca.toString(),
      "fixture provider certificate\n",
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
