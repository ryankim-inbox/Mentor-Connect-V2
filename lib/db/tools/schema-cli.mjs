...
import { runIntegrityPreflight, verifyExplainPlans } from "./integrity.mjs";
...
async function constraintsVerify(args) {
  ...
  const differences = diffCatalog(
    { tables: [], columns: [], constraints: catalog.constraints, indexes: [] },
    { tables: [], columns: [], constraints: actual.constraints, indexes: [] },
  );
  console.log(
    `constraints verify: schema ${version.schemaVersion}; target ${environment}; constraint differences ${differences.length} (read-only)`,
  );
  for (const difference of differences) console.log(`- ${difference}`);
  if (differences.length > 0) process.exitCode = 1;
}
